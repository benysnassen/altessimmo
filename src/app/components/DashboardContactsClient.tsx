...
const CreateClientForm = ({
  ...
  onCancel: () => void;
  forcedType?: "buyer" | "seller";
}) => {
  const [formData, setFormData] = useState({
    type: (forcedType ?? "buyer") as "buyer" | "seller",
    name: "",
    phone: "",
    email: "",
    budget: "",
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <input
              ...
              className={`w-full px-3 py-2 bg-white border border-black/20 rounded-sm text-black focus:border-black/50 focus:outline-none ${forcedType ? "md:col-span-2" : ""}`}
...
export type DashboardContactsKind = "BUYER" | "SELLER";

export default function DashboardContactsClient({
  contactKind,
}: {
  contactKind: DashboardContactsKind;
}) {
  ...
  const filterContacts = () => {
    let filtered = contacts.filter((c) => c.type === contactKind);
  ...
  const scoped =
    contactKind === "BUYER" ? buyers : sellers;
  const statNew = scoped.filter((c) => c.status === "NEW").length;
  const statActive = scoped.filter((c) => c.status !== "ARCHIVED").length;
  const statPipeline = scoped.filter((c) =>
    ["CONTACTED", "INTERESTED", "VIEWING", "OFFER"].includes(c.status),
  ).length;
  const statClosed = scoped.filter((c) => c.status === "SOLD").length;
  const statLabels =
    contactKind === "BUYER"
      ? {
          c1: "Nouveaux acheteurs",
          c2: "Acheteurs actifs",
          c3: "En suivi",
          c4: "Achats conclus",
        }
      : {
          c1: "Nouveaux propriétaires",
          c2: "Propriétaires actifs",
          c3: "En suivi",
          c4: "Ventes conclues",
        };
  ...
  return (
    <div className="admin-light min-h-screen bg-black text-white">
        ...
        <div className="max-w-7xl mx-auto px-0 md:px-8">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-white/60 font-light">
                {contactKind === "BUYER"
                  ? "Liste et suivi des prospects acheteurs"
                  : "Liste et suivi des mandants / propriétaires"}
              </p>
            </div>
              ...
              <div className="mt-auto">
                <div className="w-full bg-white/10 rounded-full h-1">
                  <div
                    ...
                    style={{
                      width: `${Math.min((statNew / Math.max(scoped.length, 1)) * 100, 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
              ...
              <div className="mt-auto">
                <div className="w-full bg-white/10 rounded-full h-1">
                  <div
                    ...
                    style={{
                      width: `${Math.min((statActive / Math.max(scoped.length, 1)) * 100, 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
              ...
              <div className="mt-auto">
                <div className="w-full bg-white/10 rounded-full h-1">
                  <div
                    ...
                    style={{
                      width: `${Math.min((statPipeline / Math.max(scoped.length, 1)) * 100, 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
              ...
              <div className="mt-auto">
                <div className="w-full bg-white/10 rounded-full h-1">
                  <div
                    ...
                    style={{
                      width: `${Math.min((statClosed / Math.max(scoped.length, 1)) * 100, 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
      ...
      >
        ...
        {filteredContacts.length > 0 && (
          <Pagination
            ...
            entityLabel={
              contactKind === "BUYER" ? "acheteurs" : "propriétaires"
            }
          />
        )}
      </motion.div>
      ...
      {showCreateClientModal && (
        <CreateClientForm
          forcedType={contactKind === "BUYER" ? "buyer" : "seller"}
